...
**Parity target:** with identical model / inputs / BCs, web-mbd and OpenRadioss should be bitwise identical (`Object.is` on shape metrics and ID-aligned nodal coords). Float64 `/STATE` `.sta` (E20.13) is the production compare path (not anim VTK); E20.13 round-trip alone prevents coord `Object.is` (~10⁻¹⁴ m), so the remaining gate is a **shared OR H8C/LAW2 force kernel**. Current CFL=0.9 adaptive residual is ~0.0005% Rf / ~0.00045% Lf (~0.10 μm max ID-aligned nodal gap; `bitwiseEqual` / `coordsBitwiseEqual: false`) after matching Radioss `resol` CD order (FORINT→DT12→RWALL→V→X, no double-kick), infinite-plane `/RWALL` ITIED=0 (`rgwall.F`), H8C PXC Icpre, SMAX `/DT`, DSV `vol0`, Radioss variable-dt (`DT12=½(DT1+DT2)`, `DT2≤1.1·DT2OLD`), and Radioss PG quadrature.
